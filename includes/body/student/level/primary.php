...
<table class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
   ...
   <?php
      $stmt = $auth_user->runQuery("SELECT * FROM `primaryschoolmarks` WHERE `classname`='$students->class'  AND `term`='$schoolsession->term' AND `year`='$schoolsession->year' AND `classdetails`='$students->classdetails' AND `studentid`='".$userRow['user_name']."' ");
    $stmt->execute(array());
    $primaryschoolmarks=$stmt->fetchAll(PDO::FETCH_OBJ);
    foreach ($primaryschoolmarks as $primaryschoolmarks) {
      //$schoolsession->term.''; echo $schoolsession->year.'';
        ?> 
